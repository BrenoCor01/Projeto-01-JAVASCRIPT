...
buscaBtn.addEventListener("click", () => {
  ...
  fetch(finalURL)
    ...
    .then((data) => {
      // console.log(data[0].translations.por.common);
      ...
      resultado.innerHTML = `<img src="${data[0].flags.svg}" class="bandeira-img"> 
        <h2>${data[0].translations.por.common}</h2> 
        <section> 
        <div> 
        <h4>Capital:</h4> 
        ...
        <section> 
        <div> 
        <span><a href="${data[0].maps.googleMaps}" target="_blank">Clique para ver a localização</a></span>
        </div>
        </section>
        `;
    }).catch(() => {
